/****************************************************************************
** Meta object code from reading C++ file 'NetworkRequest.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.12)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../src/3rdparty/WebLoader/src/NetworkRequest.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'NetworkRequest.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.12. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_NetworkRequest_t {
    QByteArrayData data[10];
    char stringdata0[128];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_NetworkRequest_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_NetworkRequest_t qt_meta_stringdata_NetworkRequest = {
    {
QT_MOC_LITERAL(0, 0, 14), // "NetworkRequest"
QT_MOC_LITERAL(1, 15, 14), // "uploadProgress"
QT_MOC_LITERAL(2, 30, 0), // ""
QT_MOC_LITERAL(3, 31, 16), // "downloadProgress"
QT_MOC_LITERAL(4, 48, 16), // "downloadComplete"
QT_MOC_LITERAL(5, 65, 8), // "finished"
QT_MOC_LITERAL(6, 74, 5), // "error"
QT_MOC_LITERAL(7, 80, 20), // "downloadCompleteData"
QT_MOC_LITERAL(8, 101, 9), // "slotError"
QT_MOC_LITERAL(9, 111, 16) // "slotErrorDetails"

    },
    "NetworkRequest\0uploadProgress\0\0"
    "downloadProgress\0downloadComplete\0"
    "finished\0error\0downloadCompleteData\0"
    "slotError\0slotErrorDetails"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_NetworkRequest[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    2,   54,    2, 0x06 /* Public */,
       3,    2,   59,    2, 0x06 /* Public */,
       4,    2,   64,    2, 0x06 /* Public */,
       5,    0,   69,    2, 0x06 /* Public */,
       6,    2,   70,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       7,    1,   75,    2, 0x08 /* Private */,
       8,    2,   78,    2, 0x08 /* Private */,
       9,    1,   83,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int, QMetaType::QUrl,    2,    2,
    QMetaType::Void, QMetaType::Int, QMetaType::QUrl,    2,    2,
    QMetaType::Void, QMetaType::QByteArray, QMetaType::QUrl,    2,    2,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString, QMetaType::QUrl,    2,    2,

 // slots: parameters
    QMetaType::Void, QMetaType::QByteArray,    2,
    QMetaType::Void, QMetaType::QString, QMetaType::QUrl,    2,    2,
    QMetaType::Void, QMetaType::QString,    2,

       0        // eod
};

void NetworkRequest::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<NetworkRequest *>(_o);
        (void)_t;
        switch (_id) {
        case 0: _t->uploadProgress((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QUrl(*)>(_a[2]))); break;
        case 1: _t->downloadProgress((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QUrl(*)>(_a[2]))); break;
        case 2: _t->downloadComplete((*reinterpret_cast< QByteArray(*)>(_a[1])),(*reinterpret_cast< QUrl(*)>(_a[2]))); break;
        case 3: _t->finished(); break;
        case 4: _t->error((*reinterpret_cast< QString(*)>(_a[1])),(*reinterpret_cast< QUrl(*)>(_a[2]))); break;
        case 5: _t->downloadCompleteData((*reinterpret_cast< const QByteArray(*)>(_a[1]))); break;
        case 6: _t->slotError((*reinterpret_cast< const QString(*)>(_a[1])),(*reinterpret_cast< const QUrl(*)>(_a[2]))); break;
        case 7: _t->slotErrorDetails((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (NetworkRequest::*)(int , QUrl );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&NetworkRequest::uploadProgress)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (NetworkRequest::*)(int , QUrl );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&NetworkRequest::downloadProgress)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (NetworkRequest::*)(QByteArray , QUrl );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&NetworkRequest::downloadComplete)) {
                *result = 2;
                return;
            }
        }
        {
            using _t = void (NetworkRequest::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&NetworkRequest::finished)) {
                *result = 3;
                return;
            }
        }
        {
            using _t = void (NetworkRequest::*)(QString , QUrl );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&NetworkRequest::error)) {
                *result = 4;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject NetworkRequest::staticMetaObject = { {
    QMetaObject::SuperData::link<QObject::staticMetaObject>(),
    qt_meta_stringdata_NetworkRequest.data,
    qt_meta_data_NetworkRequest,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *NetworkRequest::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *NetworkRequest::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_NetworkRequest.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int NetworkRequest::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void NetworkRequest::uploadProgress(int _t1, QUrl _t2)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t2))) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void NetworkRequest::downloadProgress(int _t1, QUrl _t2)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t2))) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void NetworkRequest::downloadComplete(QByteArray _t1, QUrl _t2)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t2))) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void NetworkRequest::finished()
{
    QMetaObject::activate(this, &staticMetaObject, 3, nullptr);
}

// SIGNAL 4
void NetworkRequest::error(QString _t1, QUrl _t2)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))), const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t2))) };
    QMetaObject::activate(this, &staticMetaObject, 4, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
